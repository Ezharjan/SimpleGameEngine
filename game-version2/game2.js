...
function renderer() {
	...
	infoChart.className = 'infoChart';
	yxibox.appendChild(infoChart);

	var censhu = document.createElement('div');
	censhu.className = 'censhu';
	censhu.innerHTML = `第${floor + 1}层`;
	infoChart.appendChild(censhu);

	var keynumber = document.createElement('div');
	keynumber.className = 'keynumber';
	//keynumber.innerHTML = `金钥匙:<br>${goldenKey}把<br>银钥匙:<br>${yinkey}把`;
	keynumber.innerHTML = `钥匙:<br>${goldenKey}把`;
	infoChart.appendChild(keynumber);

	var views = document.createElement('div');
	views.className = 'views';
	views.innerHTML = `生命:${hp}<br><br>攻击:${gjili}<br><br>防御:${defense}<br><br>`;
	infoChart.appendChild(views);
	...
	infoChart.appendChild(onceaggin);
...
function yidongguoceng(x, y, x1, y1) {
	if (arr[x1][y1] == 1) {
		arr[x1][y1] = 3;
		arr[x][y] = 1;
	} else if (arr[x1][y1] == 2 && goldenKey > 0) {
		arr[x1][y1] = 3;
		arr[x][y] = 1;
		goldenKey--;
		//} else if (arr[x1][y1] == 21 && goldenKey > 0) {
		//arr[x1][y1] = 3;
		//arr[x][y] = 1;
		//goldenKey--;
		...
		if (floor >= 0 && floor < mapArrays.length - 1) {
			floor++;
			arr = mapArrays[floor];
		}
	} else if (arr[x1][y1] == 52) {
		if (floor >= 1 && floor < mapArrays.length) {
			floor--;
			arr = mapArrays[floor];
		}
